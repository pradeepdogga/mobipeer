...
package myfirstapp.mobipeer.com.myfirstapp.hw;
...
import myfirstapp.mobipeer.com.myfirstapp.hw.CodecManager.Codec;
	...
	private void debug() {
		...
		for (int i=0;i<encoders.length;i++) {
			for (int j=0;j<encoders[i].formats.length;j++) {
				...
				Log.d(TAG, ">> Test "+(n++)+"/"+count+": "+mEncoderName+" with color format "+mEncoderColorFormat+" at "+mWidth+"x"+mHeight);
				...
				try {
					...
					Log.d(TAG, "SPS and PPS in b64: SPS="+mB64SPS+", PPS="+mB64PPS);
...
//					}
	...
	private long decode(boolean withPrefix) {
		...
		while (elapsed<9000000) {
			...
			Log.d(TAG, "In decode function before tying to get the decoded image");
