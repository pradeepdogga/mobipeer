...
import myfirstapp.mobipeer.com.myfirstapp.rtp.RtpSocket;
...
public class RtspClient {
	...
	public RtspClient() {
		...
		new HandlerThread("myfirstapp.mobipeer.com.myfirstapp.RtspClient"){
			@Override
	...
	/**
	 * Connects to the RTSP server to subscribe to the stream, and the effectively starts receiving video stream.
	 * You need to call {@link #setServerAddress(String, int)} and optionally {@link #setSession(Session)}
	 * and {@link #setCredentials(String, String)} before calling this.
	 * Should be called of the main thread !
	 */
		...
		for (int i=1;i<2;i++) {
			...
			if (stream != null) {
				String params = mParameters.transport==TRANSPORT_TCP ? 
						("TCP;interleaved="+2*i+"-"+(2*i+1)) : ("UDP;unicast;client_port="+(5000+2*i)+"-"+(5000+2*i+1));
